...
# ------------------------------------------------------------------------------
# 7.5  Initial Condition Helpers (Hotspot + Reaction Degrees)
# ------------------------------------------------------------------------------
R_GAS = 8.314  # J/(mol·K)

# Typical LiPo thermal runaway reaction parameters
# Format: [Ea (J/mol), A (1/s), H (W/m³)] for each of 4 reactions
DEFAULT_REACTION_PARAMS = np.array([
    # Reaction 0: SEI decomposition (Ea ~135 kJ/mol)
    [1.3508e5, 1.667e15, 2.57e5],
    # Reaction 1: Electrolyte decomposition (Ea ~150 kJ/mol)
    [1.5006e5, 2.500e13, 1.714e6],
    # Reaction 2: Separator melt/shutdown (Ea ~120 kJ/mol)
    [1.3960e5, 6.667e13, 3.140e5],
    # Reaction 3: Cathode decomposition (Ea ~200 kJ/mol)
...
def initialize_temperature_field(Nx, Ny, Nz, T_amb, trigger_temp,
    ...
    """
    Initialize 3D temperature field with a spherical hotspot trigger.
    T_2d comes from T_final[:, :, mid_z] with shape (Nx, Ny).
    """
    ...
    # Create spherical hotspot with smooth Gaussian-like transition
    for i in range(Nx):
        for j in range(Ny):
            for k in range(Nz):
                ...
                if dist <= trigger_radius:
                    ...
                    T[i, j, k] = trigger_temp - (trigger_temp - T_amb) * r_norm**2
...
def initialize_reaction_degrees(Nx, Ny, Nz, trigger_radius, trigger_center=None):
    """
    Initialize reaction degree-of-conversion arrays (alphas).
    All start at 1.0 (unreacted) except in hotspot where some pre-reaction occurs.
    """
    ...
    # In hotspot, slightly advance reaction 0 (SEI decomposition)
    for i in range(Nx):
        for j in range(Ny):
            for k in range(Nz):
                dist = np.sqrt((i - cx)**2 + (j - cy)**2 + (k - cz)**2)
                if dist <= trigger_radius:
                    r_norm = dist / max(trigger_radius, 1)
                    alphas[0, i, j, k] = 0.95 - 0.3 * r_norm
...
def create_multi_slice_3d_visualization(T_3d, extents, style_params,
    ...
    FIXED:
      ...
      5. Full style_params integration for fonts, colors, grid, etc.
    """
    Nx, Ny, Nz = T_3d.shape
    ext_x = extents['x']; ext_y = extents['y']; ext_z = extents['z']
    x = np.linspace(ext_x[0], ext_x[1], Nx)
    ...
    # Colorbar with full style_params integration
    colorbar_config = dict(
        title=dict(
            text='Temperature (K)',
            side='right',
    ...
    for idx, kz in enumerate(z_slices):
        ...
        # Mesh lines with customizable style
        if style_params.get('show_grid', True):
    ...
    # Apply ALL style parameters to layout
    margin = max(ext_x[1]-ext_x[0], ext_y[1]-ext_y[0], ext_z[1]-ext_z[0]) * 0.1
...
def run_simulation(params, progress_callback=None):
    ...
    # ✅ FIXED: Use proper initialization functions with smooth hotspot
    ...
    if sims:
        ...
        with st.expander("⚙️ 3D Visualization Controls", expanded=True):
            ...
            with tab1:
                ...
                # Get current time from latest history entry
                current_t = sim_data['history'][-1]['time'] if sim_data['history'] else 0.0
