...
# =============================================================================
# Fully upgraded with all advanced features from the Ag nanoparticle reference:
...
class PublicationEnhancer:
    ...
    def add_scale_bar(ax, length_physical, location='lower right',
        ...
        """Add a physical scale bar to an image-like axis."""
...
class EnhancedFigureStyler(FigureStyler):
    ...
    def apply_publication_styling(fig, axes, style_params):
        ...
        # Adjust layout with pad/wspace/hspace
...
# -----------------------------------------------------------------------------
# 9. Simulation Runner – with progress bar
# -----------------------------------------------------------------------------
...
def create_publication_heatmaps(simulations, frames, config, style_params):
    ...
    for idx, (sim, frame_idx) in enumerate(zip(simulations, frames)):
        ...
        # Add scale bar (physical length = 0.01 m for example)
...
def create_publication_statistics(simulations, frames, config, style_params):
    """Enhanced statistics with violin, KDE, cumulative, table."""
    ...
    # Second row for residuals if we have multiple sims and want correlation? We'll add a scatter of T vs alpha
    ...
    if len(simulations) > 0:
        ...
        # Q-Q plot of T distribution (normal reference)
...
def create_evolution_timeline_plot(simulations, config, style_params):
    ...
    for sim, color in zip(simulations, colors):
        ...
        # Optionally add error/confidence? Not applicable.
...
def create_cross_correlation_plot(simulations, config, style_params):
    """Scatter plot of parameter vs final Tmax with regression line and residuals."""
...
if operation_mode == "Run New Simulation":
    ...
    # Show latest interactive 3D if exists
    if sims:
        latest_id = sims[-1]['id']
    ...
    if not sims:
        ...
        if selected_ids:
            ...
            # Dynamic controls based on comparison type
            profile_direction = 'z'
            ...
            if st.sidebar.button("🔬 Run Comparison", type="primary"):
                ...
                # Real‑time post‑processing toggles (refresh styling)
...
st.caption("🔥 Multi‑Simulation Thermal Runaway Platform • 2025 • Fully upgraded")
