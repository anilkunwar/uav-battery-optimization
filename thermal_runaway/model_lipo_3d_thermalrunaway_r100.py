...
# Reaction parameter presets
REALISTIC_REACTION_PARAMS = np.array([
    [1.3508e5, 1.667e15, 5.14e5],      # SEI
    [1.5006e5, 2.500e13, 3.428e6],     # Electrolyte
    [1.3960e5, 6.667e13, 6.280e5],     # Separator
    [2.0000e5, 5.700e15, 3.100e5],     # Cathode
], dtype=np.float64)

AGGRESSIVE_REACTION_PARAMS = np.array([
    [1.3508e5, 5.000e15, 8.00e5],
    [1.5006e5, 1.000e14, 5.00e6],
    [1.3960e5, 2.000e14, 1.00e6],
    [2.0000e5, 1.000e16, 5.00e5],
], dtype=np.float64)
...
def create_multi_slice_3d_visualization(T_3d, extents, style_params,
    ...
    # Generate slices that avoid boundaries
    if Nz > 2:
    ...
    if show_cross_slices:
        ...
        fig.add_trace(go.Surface(
            ...
            colorscale=pl_colorscale, cmin=cmin, cmax=cmax,
            showscale=False,
...
def create_2d_heatmap_with_mesh(T_2d, extents_xy, style_params,
    ...
    if show_mesh:
        ...
        pcm = ax.pcolormesh(X, Y, T_2d, cmap=cmap_name,
                             shading='nearest',
                             vmin=vmin, vmax=vmax)
...
def create_publication_heatmaps(simulations, frames, config, style_params):
    ...
    # Compute global range across all selected simulations (for fair comparison)
    if style_params.get('use_custom_cbar_range', False):
...
if operation_mode == "Run New Simulation":
    ...
    with st.sidebar.expander("🔬 Reaction Kinetics Preset", expanded=True):
        ...
        if reaction_preset == 'Realistic (Recommended)':
            ...
            st.info("✅ Should reach 600‑900 K in 5‑15 min")
    ...
    with st.sidebar.expander("🔍 Pre‑Simulation Diagnostics", expanded=False):
        ...
        if trigger_temp < 420:
            st.error("⚠️ Trigger too low! Use ≥ 420 K.")
        elif trigger_temp < 450:
            st.warning("⚠️ Lower end – consider 450+ K.")
        else:
    ...
    # ───── NEW: auto-fill T_min/T_max from global field ─────
    if advanced_styling.get('cbar_auto_from_global', False) and sims:
        latest_id_auto = sims[-1]['id']
        sim_data_auto = SimulationDB.get_all_simulations()[latest_id_auto]
        T_global = sim_data_auto['final_3D'][0]
    ...
    if sims:
        ...
        with st.expander("⚙️ 3D Visualization Controls", expanded=True):
            ...
            with tabs[1]:
                # Single slice with wireframe (no time slider, uses final state)
                col1, col2 = st.columns(2)
                with col1:
            ...
            with tabs[2]:
                ...
                # ───── NEW: build iso levels from user range or data ─────
            ...
            with tabs[3]:
                # 2D heatmap with mesh (no time slider, uses final state)
