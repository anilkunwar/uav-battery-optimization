...
# =============================================================================
# UPGRADED VERSION 3.5.1 – Smooth volumetric slicing via interpolation
# =============================================================================
# All features from v3.5.0 retained; now you can render a continuous volume.
# =============================================================================
...
*Upgraded v3.5.1 — Smooth volumetric slicing via interpolation*
""")
...
# -----------------------------------------------------------------------------
# 8.6 Visualization Functions – with interpolation for smooth multi‑slice
# -----------------------------------------------------------------------------
...
# ----- NEW: Interpolated multi-slice function -----
def create_multi_slice_3d_visualization(T_3d, extents, style_params,
    ...
    # Allow n_slices > Nz by interpolating between grid points
    if Nz > 2:
        z_pos_values = np.linspace(z[1], z[-2], n_slices)
    else:
        z_pos_values = np.array([z[Nz//2]])
        
    n_actual_slices = len(z_pos_values)
    fig = go.Figure()
    colorbar_config = dict(
        title=dict(
            text='Temperature (K)',
    ...
    for idx, z_val in enumerate(z_pos_values):
        X, Y = np.meshgrid(x, y, indexing='ij')
        Z_pos = np.full_like(X, z_val)
        
        # Use interpolation if between grid points, else grab exact slice (faster)
        if np.any(np.isclose(z, z_val, atol=1e-12)):
            kz = int(np.argmin(np.abs(z - z_val)))
            T_slice = T_3d[:, :, kz]
        else:
            pts = np.array([X.flatten(), Y.flatten(), np.full(X.size, z_val)]).T
            T_slice = interpolate.interpn((x, y, z), T_3d, pts, method='linear', bounds_error=False, fill_value=None)
            T_slice = T_slice.reshape(Nx, Ny)
            
        opacity = 0.5 + 0.4 * ((z_val - z[0]) / max(z[-1]-z[0], 1e-6))
        ...
        if style_params.get('show_grid', True):
            ...
            for i in range(0, Nx, step_x):
                fig.add_trace(go.Scatter3d(
                    x=[x[i], x[i]], y=[ext_y[0], ext_y[1]], z=[z_val, z_val],
                    mode='lines', line=dict(color=mesh_color, width=mesh_width),
                    opacity=mesh_opacity, showlegend=False, hoverinfo='skip'
                ))
            for j in range(0, Ny, step_y):
                fig.add_trace(go.Scatter3d(
                    x=[ext_x[0], ext_x[1]], y=[y[j], y[j]], z=[z_val, z_val],
                    mode='lines', line=dict(color=mesh_color, width=mesh_width),
                    opacity=mesh_opacity, showlegend=False, hoverinfo='skip'
                ))
...
# -----------------------------------------------------------------------------
# Remaining plotting functions (unchanged)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 12. Main UI – with Drone Battery Model integration and interpolated slicing
# -----------------------------------------------------------------------------
        ...
        with st.expander("⚙️ 3D Visualization Controls", expanded=True):
            ...
            with tabs[0]:
                if has_snapshots:
                    ...
                    with col2:
                        # Allow more slices than Nz; max 50 for performance
                        n_slices = st.slider(
                            "Z‑slices (interpolated)",
                            1, 50, min(5, max(1, n_snapshots//2)),
                            key='n_slices_ms',
                            help="You can now select more slices than grid points (Nz) for a smoother 3D volumetric look."
                        )
                    with col3:
...
# -----------------------------------------------------------------------------
with st.expander("🔬 Theoretical Soundness & Advanced Analysis (v3.5.1)", expanded=False):
    st.markdown("""
    ...
    **Smooth Volumetric Slicing (NEW)**
    - Uses 3D linear interpolation (`scipy.interpolate.interpn`) to render any number of Z‑slices, even exceeding the grid resolution `Nz`.
    - Creates a continuous, realistic volume appearance rather than discrete blocky layers.
    - Slider now allows up to 50 slices for near‑solid rendering.
...
st.caption("🔥 Multi‑Simulation Thermal Runaway Platform • v3.5.1 • Smooth volumetric slicing • Drone Battery Models • Custom color‑bar")
