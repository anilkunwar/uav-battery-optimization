...
# -----------------------------------------------------------------------------
# 8. Numba Kernel – UPDATED with boundary heater
# -----------------------------------------------------------------------------
        ...
        for j in prange(1, Ny - 1):
            for k in prange(1, Nz - 1):
                ...
                for r in range(4):
                    ...
                    alphas_new[r,i,j,k] = max(alpha + dalpha, 0.0)  # α increases with conversion
...
# -----------------------------------------------------------------------------
# 12. Main UI – UPDATED with Heater Controls
# -----------------------------------------------------------------------------
...
if operation_mode == "Run New Simulation":
    ...
    with st.sidebar.expander("🔥 Trigger & Heater", expanded=True):
        col1, col2 = st.columns(2)
        with col1:
            trigger_temp = st.number_input("Hotspot T (K)", 0, 2000, 0, 5,
                                           help="Set to 0 to disable point hotspot")
            trigger_radius = st.slider("Hotspot radius (cells)", 0, 10, 3, disabled=(trigger_temp==0))
        with col2:
            heater_enabled = st.checkbox("Enable Boundary Heater (X=0 face)", value=True,
                                         help="Simulates lateral overheating from adjacent cell")
    ...
    with st.sidebar.expander("🔬 Reaction Kinetics Preset", expanded=True):
        ...
        if reaction_preset == 'Realistic (Recommended)':
            ...
            st.info("✅ Should reach 600‑900 K in 5‑15 min (with heater)")
    ...
    with st.sidebar.expander("🔍 Pre‑Simulation Diagnostics", expanded=False):
        ...
        if trigger_temp > 0:
            ...
            if trigger_temp < 420:
                st.error("⚠️ Trigger too low! Use ≥ 420 K or rely on heater.")
        if heater_enabled:
            st.write(f"**Heater flux:** {q_heater} W/m², cutoff at {heater_cutoff_temp} K")
            if q_heater < 10000:
                st.warning("Low heater flux may not initiate runaway.")
...
with st.expander("🔬 Theoretical Soundness & Advanced Analysis (v3.0)", expanded=False):
    ...
    **Multi‑Stage Arrhenius Kinetics** (corrected α evolution)
    - **SEI decomposition** (r=0): first‑order in α
    - **Anode (electrolyte)** (r=1): autocatalytic α(1‑α)
    ...
    **Boundary Heating (Lateral Overheating)**
    - A constant heat flux $q_0$ is applied to the X=0 face (simulating a neighbouring cell's exothermic reaction).
    ...
    **Simulation Conditions**
    ...
    - `trigger_temp` = 0 (point hotspot disabled) – instead the boundary heater initiates runaway.
...
st.caption("🔥 Physics‑Corrected v3.0 • Validated against Zhang et al. 2024 • Boundary heater + scaled enthalpies • Click 'Run & Save' to start.")
