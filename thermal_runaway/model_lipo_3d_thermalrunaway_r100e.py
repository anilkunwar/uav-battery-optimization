...
if operation_mode == "Run New Simulation":
    ...
    with st.sidebar.expander("Heat & Trigger", expanded=True):
        ...
        trigger_temp = st.number_input("Hotspot T (K)", 250, 2000, 450, 5,
                                       help="Set to ambient to disable hotspot; heater will dominate")
        trigger_radius = st.slider("Hotspot radius (cells)", 0, 10, 3,
                                   help="Set to 0 to disable internal hotspot")

    with st.sidebar.expander("🔥 Sustained Heater (Abuse Protocol)", expanded=True):
        use_heater = st.checkbox("Enable Boundary Heater", value=True,
        ...
        q_heater = st.number_input("Heater Flux (W/m²)", 0.0, 5e5, 3e4, 5e3, format="%.0f",
                                   help="Typical range 3e4–1e5 W/m²")
        heater_cutoff_temp = st.number_input("Heater Cutoff (K)", 300, 1000, 500, 10,
                                             help="Heater turns off when T_max exceeds this")
        ...
        heater_face_val = heater_face_map[heater_face]
    ...
    with st.sidebar.expander("🔍 Pre‑Simulation Diagnostics", expanded=False):
        ...
        if trigger_temp < 420 and trigger_radius > 0:
            st.error("⚠️ Trigger too low! Use ≥ 420 K or disable hotspot (radius=0).")
        elif trigger_temp < 450 and trigger_radius > 0:
            st.warning("⚠️ Lower end – consider 450+ K or rely on heater.")
        else:
    ...
    # ───── NEW: “Use Global Min/Max” button logic ──────────────────────────
...
with st.expander("🔬 Theoretical Soundness & Advanced Analysis", expanded=False):
    ...
    **Sustained Boundary Heat Flux (Abuse Protocol)**
...
st.caption("🔥 Multi‑Simulation Thermal Runaway Platform • Upgraded 3.0 • Realistic kinetics + sustained heat‑flux abuse")
