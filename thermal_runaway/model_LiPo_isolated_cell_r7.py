...
# =============================================================================
# NEW: Multi‑Slice 3D Visualisation with optional cross‑slices
# =============================================================================
def create_multi_slice_3d_visualization(T_3d, extents, style_params,
                                        n_slices=5, show_cross_slices=False):
    """
    Create a 3D Plotly figure with multiple horizontal Z‑slices and optional
    vertical X‑Y cross‑planes through the domain centre.
    """
    Nx, Ny, Nz = T_3d.shape
    X = np.linspace(extents['x'][0], extents['x'][1], Nx)
    Y = np.linspace(extents['y'][0], extents['y'][1], Ny)
    Z = np.linspace(extents['z'][0], extents['z'][1], Nz)

    # Choose a plotly colorscale from the current styling
    cmap_name = style_params.get('cmap', 'inferno')
    pl_colorscale = matplotlib_to_plotly(cmap_name)

    fig = go.Figure()

    # 1. Horizontal Z‑slices (equally spaced)
    if n_slices > Nz:
        n_slices = Nz  # avoid more slices than grid points
    z_indices = np.linspace(0, Nz-1, n_slices, dtype=int)
    for zi in z_indices:
        z_phys = Z[zi]
        slice_data = T_3d[:, :, zi]
        fig.add_trace(go.Surface(
            x=X, y=Y, z=np.full_like(X[:, None], z_phys),
            surfacecolor=slice_data,
            colorscale=pl_colorscale,
            showscale=False,
            opacity=0.85,
            hoverinfo='skip',
            name=f'z={z_phys:.3f} m'
        ))

    # 2. Optional vertical cross‑slices (the "cross")
    if show_cross_slices:
        # Y‑direction slice (constant Y = centre)
        ky = Ny // 2
        Y_pos = Y[ky]
        data_y = T_3d[:, ky, :]
        fig.add_trace(go.Surface(
            x=X, y=np.full_like(X, Y_pos), z=Z[np.newaxis, :],
            surfacecolor=data_y,
            colorscale=pl_colorscale,
            showscale=False,
            opacity=0.25,          # translucent to avoid solid wall appearance
            hoverinfo='skip',
            name='Y‑cross (centre)'
        ))
        # X‑direction slice (constant X = centre)
        kx = Nx // 2
        X_pos = X[kx]
        data_x = T_3d[kx, :, :]
        fig.add_trace(go.Surface(
            x=np.full_like(Y, X_pos), y=Y, z=Z[np.newaxis, :],
            surfacecolor=data_x.T,
            colorscale=pl_colorscale,
            showscale=False,
            opacity=0.25,
            hoverinfo='skip',
    ...
    # Add a colorbar for the whole figure (use one of the traces)
    fig.update_traces(
        colorbar=dict(
            title='Temperature (K)',
            len=0.8,
    ...
    fig.update_layout(
        ...
        title=dict(text='Multi‑Slice 3D Thermal Field', font=dict(size=16)),
        height=700,
...
if operation_mode == "Run New Simulation":
    ...
    if sims:
        ...
        with st.expander("⚙️ 3D Visualization Controls", expanded=True):
            ...
            with col2:
                # For Multi-Slice, we need number of slices and cross checkbox
                if viz_type == "Multi-Slice":
                    # Use Nz to set max slices (ensure at least 2)
                    max_slices = max(2, Nz - 2)
        ...
        if viz_type == "Volumetric":
            ...
            fig3d = create_multi_slice_3d_visualization(
                T_final, ext, advanced_styling,
                n_slices=n_slices,
                show_cross_slices=show_cross
            )
            # We already set the layout inside the function, but we might want to override font size
            fig3d.update_layout(
                scene=dict(
                    xaxis=dict(
                ...
                title=dict(text='🔥 3D Thermal Field (Multi-Slice)', x=0.5, font=dict(size=font_size_3d+4)),
                height=700,
                margin=dict(l=0, r=0, b=0, t=40)
            )

        # For Volumetric and Isosurfaces, we still need to add the layout
        if viz_type != "Multi-Slice":
            fig3d.update_layout(
                scene=dict(
                    xaxis=dict(
