...
# =============================================================================
# UPGRADED VERSION 3.0 (PHYSICS + VISUALIZATION FIXES):
#   FIX 1  – Kinetic lock removed: reaction degrees now use CONVERSION
#            convention c ∈ [0,1] (0 = unreacted). Autocatalytic reaction is
#            globally seeded (c0 = 1e-4) so it can ignite.
#   FIX 2  – Enthalpies rescaled to reality: ΣH ≈ 1.42e9 J/m³ → adiabatic
#            ΔT ≈ 550 K (Tmax 600–900 K, matching Zhang et al. 2024 LFP stats).
#   FIX 3  – Pre‑exponential factors calibrated so self‑heat onset falls in
#            the 70–135 °C window (paper Table 2) and dT/dt > 1 K/s at TR.
#   FIX 4  – Energy‑conserving reaction integration: q = H·Δc/Δt with exact
#            exponential update (1st order) and capped explicit (autocatalytic)
#            → unconditionally stable, dt‑independent heat release.
#   FIX 5  – Real abuse protocols (simulation conditions):
#              • Lateral heater  – sustained flux q0 on x=0 face (paper Eq. 2),
#                auto‑off at cutoff temperature.
#              • ARC adiabatic   – bulk pre‑heat, h_conv = eps = 0.
#              • Internal hotspot – enlarged/seeded spherical trigger (ISC).
#   FIX 6  – Visualization: fixed color scale across the time slider, dynamic
#            isosurface levels, correct LFP reference lines (353/500/900 K),
#            heater face rendered in the 3D sketch, colorbar tickformat fixed.
#   All original features retained (2D/3D plots, comparison, export, styling).
# =============================================================================
...
*v3.0 – physically calibrated kinetics + real abuse protocols + fixed visualization*
""")
...
class PublicationEnhancer:
    ...
    @staticmethod
    def add_confidence_band(ax, y_data, confidence=0.95, color='blue', alpha=0.2):
        y_mean = np.mean(y_data, axis=0)
        y_std = np.std(y_data, axis=0)
        n = len(y_data)
        t_val = stats.t.ppf((1 + confidence) / 2, n - 1) if n > 1 else 0
        y_err = t_val * y_std / np.sqrt(n)
        ax.fill_between(np.arange(n), y_mean - y_err, y_mean + y_err,
...
# -----------------------------------------------------------------------------
# 6. Simulation Database (stores snapshots)
# -----------------------------------------------------------------------------
class SimulationDB:
    @staticmethod
    ...
    def get_simulation_list():
        ...
        for sim_id, data in SimulationDB.get_all_simulations().items():
            params = data['params']
            name = f"{params.get('label', 'LiPo')} {params.get('abuse_mode','heater')} h={params['h_conv']:.1f}"
...
# -----------------------------------------------------------------------------
# 7.5 Initial Condition Helpers – UPGRADED (conversion convention + seeds)
# -----------------------------------------------------------------------------
R_GAS = 8.314  # J/(mol·K)

# ----------------------------------------------------------------------------
# CALIBRATED reaction presets (v3.0)
#   Columns: [Ea (J/mol), A (1/s), H (J/m³)]
#   • H rescaled ×~1000 → ΣH ≈ 1.42e9 J/m³ → adiabatic ΔT ≈ 550 K
#     (Tmax ≈ 600–900 K, matching Zhang et al. 2024 LFP statistics).
#   • A calibrated so self‑heat onset (0.02 K/min criterion) falls at
#     ≈ 345–360 K for SEI, i.e. inside the paper's 70–135 °C LFP window.
# ----------------------------------------------------------------------------
REALISTIC_REACTION_PARAMS = np.array([
    [1.3508e5, 1.000e13, 2.50e8],     # SEI decomposition (1st order)
    [1.5006e5, 1.000e12, 7.00e8],     # Anode/electrolyte (autocatalytic)
    [1.3960e5, 6.700e13, 1.20e8],     # Separator reactions (1st order)
    [2.0000e5, 5.700e13, 3.50e8],     # Cathode decomposition (1st order, >500 K)
], dtype=np.float64)

AGGRESSIVE_REACTION_PARAMS = np.array([
    [1.3508e5, 5.000e13, 3.00e8],
    [1.5006e5, 5.000e12, 8.00e8],
    [1.3960e5, 2.000e14, 1.50e8],
    [2.0000e5, 2.000e14, 4.00e8],
], dtype=np.float64)

# Original (v1/v2) values – kept ONLY for comparison; cannot produce runaway.
DEFAULT_REACTION_PARAMS = np.array([
    [1.3508e5, 1.667e15, 2.57e5],
    [1.5006e5, 2.500e13, 1.714e6],
    [1.3960e5, 6.667e13, 3.140e5],
    [2.0000e5, 5.700e15, 1.550e5],
], dtype=np.float64)

def initialize_temperature_field(Nx, Ny, Nz, T_bulk, trigger_temp,
                                 trigger_radius, use_hotspot, trigger_center=None):
    """Vectorized initialization. Bulk temperature + optional spherical hotspot."""
    T = np.full((Nx, Ny, Nz), T_bulk, dtype=np.float64)
    if use_hotspot and trigger_radius > 0:
        ...
        T[mask] = trigger_temp - (trigger_temp - T_bulk) * (r_norm[mask])**2
    return T

def initialize_reaction_degrees(Nx, Ny, Nz, trigger_radius, use_hotspot,
                                trigger_center=None):
    """
    UPGRADED – conversion convention: c = 0 → unreacted, c = 1 → fully reacted.
    • Autocatalytic reaction (r=1) gets a global physical seed c0 = 1e-4
      (nucleation sites) so α(1‑α) kinetics can ignite (FIX 1).
    • Inside the hotspot the anode reaction is pre‑seeded (c = 0.01) to mimic
      local degradation / internal short damage.
    """
    alphas = np.zeros((4, Nx, Ny, Nz), dtype=np.float64)
    alphas[1] += 1.0e-4          # global nucleation seed (autocatalytic)
    if use_hotspot and trigger_radius > 0:
        if trigger_center is None:
            cx, cy, cz = Nx // 2, Ny // 2, Nz // 2
        else:
        ...
        mask = dist <= trigger_radius
        r_norm = dist / max(trigger_radius, 1)
        alphas[1, mask] = 0.01 * (1.0 - 0.5 * r_norm[mask])   # damaged zone seed
    return alphas

# -----------------------------------------------------------------------------
# 8. Numba Kernel – UPGRADED (energy-conserving kinetics + heater face BC)
# -----------------------------------------------------------------------------
@njit(parallel=True, fastmath=True, cache=True)
def step_3d(T, alphas, dt,
            rho, Cp, kx, ky, kz, dx, dy, dz,
            q_normal, reaction_params, T_amb, h_conv, eps, sigma, R,
            q_heater, heater_active):
    """
    v3.0 kernel.
    • Reaction update is ENERGY CONSERVING: q_abuse = Σ H_r · Δc_r / dt.
      - 1st‑order reactions: exact exponential update (unconditionally stable)
        Δc = (1‑c)·(1‑exp(‑rate·dt))
      - autocatalytic (r=1): explicit with Δc cap 0.2 (stability)
    • x=0 face receives sustained heater flux q0 while heater_active (paper Eq.2).
    """
    Nx, Ny, Nz = T.shape
    T_new = T.copy()
            ...
            for k in prange(1, Nz - 1):
                ...
                for r in range(4):
                    ...
                    if r == 1:
                        ...
                        dc = rate * f * dt
                        if dc > 0.2: dc = 0.2
                        if dc > (1.0 - c): dc = 1.0 - c
                    else:
                        dc = (1.0 - c) * (1.0 - np.exp(-rate * dt))
                    alphas_new[r,i,j,k] = c + dc
                    q_abuse += H * dc / dt
    ...
    for j in prange(Ny):
        for k in prange(Nz):
            ...
            q_net = q_conv + q_rad
            if heater_active:
                q_net -= q_heater          # sustained abuse flux INTO the cell
...
# -----------------------------------------------------------------------------
# 8.5 Domain Sketch Functions – upgraded (heater face rendered)
# -----------------------------------------------------------------------------
def plot_initial_domain_sketch(params):
    ...
    if params.get('use_hotspot', False):
        cx, cz = Lx/2, Lz/2
        dx = Lx / (Nx - 1)
    ...
    if params.get('q_heater', 0.0) > 0:
        heater_rect = plt.Rectangle((-margin*0.15, 0), margin*0.15, Lz,
                                    linewidth=2, edgecolor='#d35400', facecolor='#e67e22', alpha=0.8,
                                    label='Heater face q0')
...
def plot_3d_domain_sketch(params):
    """Interactive 3D geometry sketch with physically-proportioned arrows + heater face."""
    Lx, Ly, Lz = params['Lx'], params['Ly'], params['Lz']
    Nx = params['Nx']
    r_phys = params['trigger_radius'] * (Lx / (Nx - 1)) if params.get('use_hotspot', False) else 0.0
    ...
    if params.get('q_heater', 0.0) > 0:
        yy, zz = np.meshgrid([0, Ly], [0, Lz])
        xx = np.zeros_like(yy)
        fig.add_trace(go.Surface(
            x=xx, y=yy, z=zz,
            colorscale=[[0, '#e67e22'], [1, '#e67e22']],
            showscale=False, opacity=0.6, name='Heater face (q0)'))
...
# -----------------------------------------------------------------------------
# 8.6 Visualization Functions – upgraded (stable colorbars, fixed tickformat)
# -----------------------------------------------------------------------------
def create_mesh_aware_3d_thermal(T_3d, extents, style_params,
    ...
    colorbar_config = dict(
        ...
        thickness=15, len=0.8, tickformat='.0f'
    )
    ...
    if slice_axis == 'z':
        fig.add_trace(go.Surface(
            x=X, y=Y, z=Z_pos, surfacecolor=Z_data,
            colorscale=pl_colorscale, colorbar=colorbar_config, showscale=True,
            opacity=0.9, cmin=vmin, cmax=vmax,
        ...
        fig.add_trace(go.Surface(
            x=X, y=Y_pos, z=Z, surfacecolor=Y_data,
            colorscale=pl_colorscale, colorbar=colorbar_config, showscale=True,
            opacity=0.9, cmin=vmin, cmax=vmax,
            lighting=dict(ambient=0.6, diffuse=0.4, specular=0.1)))
        if show_mesh:
        ...
        fig.add_trace(go.Surface(
            x=X_pos, y=Y, z=Z, surfacecolor=X_data,
            colorscale=pl_colorscale, colorbar=colorbar_config, showscale=True,
            opacity=0.9, cmin=vmin, cmax=vmax,
            lighting=dict(ambient=0.6, diffuse=0.4, specular=0.1)))
        if show_mesh:
    ...
    fig.update_layout(
        scene=dict(
            ...
            bgcolor=bg_color),
        title=dict(text=f'🔥 3D Thermal Field with Mesh | T: {T_min:.1f} - {T_max:.1f} K',
                   x=0.5, font=dict(size=title_size, color=title_color, family='Arial')),
...
def create_multi_slice_3d_visualization(T_3d, extents, style_params,
                                         ...
                                         current_time=0.0, vmin=None, vmax=None):
    Nx, Ny, Nz = T_3d.shape
    ext_x = extents['x']; ext_y = extents['y']; ext_z = extents['z']
    x = np.linspace(ext_x[0], ext_x[1], Nx)
    y = np.linspace(ext_y[0], ext_y[1], Ny)
    ...
    colorbar_config = dict(
        title=dict(text='Temperature (K)', side='right',
                   font=dict(size=style_params.get('colorbar_fontsize', 12),
        ...
        tickfont=dict(size=style_params.get('tick_font_size', 12),
                      color=style_params.get('title_color', '#000000')),
        tickformat='.0f')
    ...
    for idx, kz in enumerate(z_slices):
        ...
        fig.add_trace(go.Surface(
            ...
            opacity=opacity, cmin=vmin, cmax=vmax,
    ...
    if show_cross_slices:
        ...
        fig.add_trace(go.Surface(
            ...
            opacity=0.25, cmin=vmin, cmax=vmax, name='Y-center slice'))
        step_x = max(1, Nx // 10)
        step_z = max(1, Nz // 8)
        for i in range(0, Nx, step_x):
            fig.add_trace(go.Scatter3d(
        ...
        fig.add_trace(go.Surface(
            ...
            colorscale=pl_colorscale, showscale=False,
            opacity=0.25, cmin=vmin, cmax=vmax, name='X-center slice'))
        step_y = max(1, Ny // 10)
        for j in range(0, Ny, step_y):
            fig.add_trace(go.Scatter3d(
    ...
    fig.add_trace(go.Scatter3d(
        ...
        mode='markers', marker=dict(size=5, color=box_color, symbol='diamond'),
    ...
    fig.update_layout(
        scene=dict(
            ...
            camera=dict(eye=dict(x=1.5, y=1.5, z=0.8)),
            bgcolor=bg_color),
        title=dict(text=f'🔥 Multi-Slice 3D Thermal Field | t = {current_time:.1f} s | T: {T_min:.1f} - {T_max:.1f} K | Mesh: {Nx}×{Ny}×{Nz} | {n_actual_slices} Z-slices',
                   x=0.5, font=dict(size=title_size, color=title_color, family='Arial')),
...
def create_2d_heatmap_with_mesh(T_2d, extents_xy, style_params,
    ...
    if show_mesh:
        pcm = ax.pcolormesh(X, Y, T_2d, cmap=cmap_name, shading='nearest',
                            edgecolors=mesh_color, linewidth=mesh_linewidth,
...
# -----------------------------------------------------------------------------
# 9. Simulation Runner – upgraded (abuse protocols + heater control)
# -----------------------------------------------------------------------------
def run_simulation(params, progress_callback=None):
    ...
    # ---- v3.0 abuse protocol handling ----
    abuse_mode = params.get('abuse_mode', 'heater')
    q_heater = params.get('q_heater', 6.0e4) if abuse_mode == 'heater' else 0.0
    heater_cutoff_temp = params.get('heater_cutoff_temp', 500.0)
    T_bulk_init = params.get('T_bulk_init', T_amb)
    use_hotspot = (abuse_mode == 'hotspot')
    if abuse_mode == 'arc':          # adiabatic ARC override
        h_conv = 0.0
        eps = 0.0
    ...
    adapt_dt_thresh = params.get('adapt_dt_thresh', 450.0)
    ...
    T = initialize_temperature_field(
        Nx, Ny, Nz, T_bulk=T_bulk_init, trigger_temp=trigger_temp,
        trigger_radius=trigger_radius, use_hotspot=use_hotspot)
    alphas = initialize_reaction_degrees(
        Nx, Ny, Nz, trigger_radius=trigger_radius, use_hotspot=use_hotspot)
    ...
    t = 0.0; step = 0
    times = []; T_max_history = []; T_avg_history = []
    ...
    while t < sim_time:
        ...
        # Heater active while the heated face is below cutoff (paper protocol)
        heater_active = heater_on and (np.max(T[0, :, :]) < heater_cutoff_temp)
        ...
        if t >= sample_next:
            ...
            T_avg_history.append(float(np.mean(T)))
...
# -----------------------------------------------------------------------------
# 10. Enhanced Plotting Functions (reference lines upgraded)
# -----------------------------------------------------------------------------
...
def create_publication_statistics(simulations, frames, config, style_params):
    ...
    for data, label in zip(all_data, labels):
        stats_data.append([
            label, len(data), f"{np.mean(data):.1f}", f"{np.std(data):.1f}",
            f"{np.median(data):.1f}", f"{np.max(data):.1f}",
    ...
    if len(simulations) > 0:
        ...
        ax_corr.set_ylabel("SEI conversion α")
...
def create_evolution_timeline_plot(simulations, config, style_params):
    ...
    ax.axhline(353, color='orange', linestyle='--', label='LFP self-heat onset (~80°C)')
    ax.axhline(500, color='darkred', linestyle='--', label='Runaway threshold (~227°C)')
    if style_params.get('show_legend', True):
...
# -----------------------------------------------------------------------------
# 11. Time‑series plot with marker – upgraded reference lines (LFP physics)
# -----------------------------------------------------------------------------
def create_time_series_with_marker(sim_data, current_time, style_params):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=times, y=Tmax, mode='lines', name='T_max',
                             line=dict(color='red', width=3)))
    fig.add_vline(x=current_time, line_dash="dash", line_color="green",
                  annotation_text=f"t={current_time:.1f}s")

    # Physically meaningful LFP reference temperatures (Zhang et al. 2024)
    fig.add_hline(y=353, line_dash="dot", line_color="orange",
                  annotation_text="LFP self-heat onset (~80°C)")
    fig.add_hline(y=500, line_dash="dot", line_color="red",
                  annotation_text="Runaway onset (~227°C)")
    fig.add_hline(y=900, line_dash="dot", line_color="darkred",
                  annotation_text="Typical LFP Tmax (~627°C)")
...
if operation_mode == "Run New Simulation":
    ...
    # ---- v3.0 Abuse Protocol ----
    with st.sidebar.expander("🔥 Abuse Protocol (simulation conditions)", expanded=True):
        abuse_mode = st.selectbox(
            "Abuse scenario",
            ['heater', 'arc', 'hotspot'],
            format_func=lambda m: {
                'heater': 'Lateral heater (Zhang et al. Fig.1b)',
                'arc': 'ARC adiabatic (bulk pre‑heat)',
                'hotspot': 'Internal hotspot (ISC / nail)'}[m],
            index=0)
        if abuse_mode == 'heater':
            q_heater = st.slider("Heater flux q0 (W/m²)", 0, 300000, 60000, 5000,
                                 help="Paper uses sustained lateral heating until TR")
            heater_cutoff_temp = st.slider("Heater auto‑off at face T (K)", 350, 900, 500, 10)
            T_bulk_init = st.number_input("Initial bulk T (K)", 250, 450, 298, 1)
            st.info("✅ Sustained flux on x=0 face, auto‑off at cutoff → reproduces the paper's 4 stages.")
        elif abuse_mode == 'arc':
            q_heater = 0.0
            heater_cutoff_temp = 9999.0
            T_bulk_init = st.number_input("Initial bulk T (K)", 250, 450, 373, 1,
                                          help="ARC tests start at 40–70°C; adiabatic (h=ε=0) forced")
            st.info("✅ Adiabatic: h_conv and ε forced to 0. Expect onset 343–408 K (LFP).")
        else:
            q_heater = 0.0
            heater_cutoff_temp = 9999.0
            T_bulk_init = st.number_input("Initial bulk T (K)", 250, 450, 298, 1)
            st.warning("⚡ Hotspot must be ≥ 600 K and ≥ 5 cells to exceed critical trigger energy.")
    ...
    with st.sidebar.expander("Material & Boundary"):
        ...
        h_conv = st.number_input("h_conv (W/m²·K)", 0.0, 50.0, 7.0, 1.0,
                                 help="Use ≈0‑5 for near‑adiabatic; 10‑15 natural convection")
    ...
    with st.sidebar.expander("Heat & Trigger", expanded=True):
        q_normal = st.number_input("Normal Heat (W/m³)", 0.0, 5e5, 0.0, 1e4, format="%.0f",
                                   help="Internal ohmic baseline; keep 0 for pure abuse study")
        trigger_temp = st.number_input("Hotspot T (K)", 350, 2000, 650, 5,
                                       help="≥600 K needed for self‑sustaining ignition (hotspot mode)")
        trigger_radius = st.slider("Hotspot radius (cells)", 1, 10, 5)
    ...
    with st.sidebar.expander("⚙️ Advanced Numerics", expanded=False):
        ...
        adapt_dt_thresh = st.slider("Adaptive dt Threshold (K)", 400, 1000, 450, 10)
    ...
    with st.sidebar.expander("🔬 Reaction Kinetics Preset", expanded=True):
        ...
        if reaction_preset == 'Realistic (Recommended)':
        ...
        elif reaction_preset == 'Original (Conservative – no TR)':
            reaction_params = DEFAULT_REACTION_PARAMS.copy()
            st.error("⚠️ v1/v2 values: ΣH gives only ~2 K adiabatic rise – cannot run away (kept for comparison).")
        else:
            reaction_params = REALISTIC_REACTION_PARAMS.copy()
            st.info("Adjust each reaction below (H in J/m³!).")
            with st.expander("Custom Reaction Parameters"):
                for i, name in enumerate(['SEI', 'Electrolyte', 'Separator', 'Cathode']):
                    ...
                    with c3:
                        reaction_params[i,2] = st.number_input(f"H{i} (J/m³)", value=float(reaction_params[i,2]), format='%e', key=f'h_{i}')

    label = st.sidebar.text_input("Run Label (optional)", value=f"{abuse_mode} h={h_conv:.1f}")

    # ---- Pre‑sim diagnostics (v3.0 energy budget) ----
    with st.sidebar.expander("🔍 Pre‑Simulation Diagnostics", expanded=False):
        total_H = float(reaction_params[:, 2].sum())
        adiab_rise = total_H / (rho * Cp)
        st.write(f"**Total chemical energy:** {total_H:.2e} J/m³")
        st.write(f"**Adiabatic ΔT potential:** ≈ {adiab_rise:.0f} K")
        if adiab_rise < 50:
            st.error("⚠️ Enthalpies too small – runaway impossible (need ΣH ≥ 1e8 J/m³).")
        else:
            st.success("✅ Enthalpy scale realistic (500–600 K rise).")
        st.write(f"**Trigger T:** {trigger_temp} K = {trigger_temp-273.15:.0f} °C")
        st.write(f"**Ambient:** {T_amb} K | **Bulk init:** {T_bulk_init} K")
        if abuse_mode == 'hotspot':
            dx_p = Lx / (Nx - 1)
            V_hot = 4/3 * np.pi * (trigger_radius * dx_p)**3
            E_trig = rho * Cp * V_hot * max(trigger_temp - T_bulk_init, 0)
            st.write(f"**Hotspot energy:** ≈ {E_trig:.0f} J (kJ‑scale needed for TR)")
        if abuse_mode == 'heater':
            st.write(f"**Heater power:** ≈ {q_heater * Ly * Lz:.1f} W into x=0 face")
    ...
    sketch_params = {
        ...
        'use_hotspot': (abuse_mode == 'hotspot'),
        'q_heater': q_heater if abuse_mode == 'heater' else 0.0
    }
    ...
    if st.sidebar.button("🚀 Run & Save", type="primary"):
        params = {
            ...
            'abuse_mode': abuse_mode,
            'q_heater': q_heater,
            'heater_cutoff_temp': heater_cutoff_temp,
            'T_bulk_init': T_bulk_init,
            'label': label,
        ...
        progress_bar.empty()
        live_metrics.success(f"✅ Done in {metadata['efficiency']['wall_time_s']:.2f}s – {len(snapshots_3d)} snapshots stored | Final Tmax {metadata['final_T_max']:.0f} K")
        time.sleep(0.5)
        st.rerun()
    ...
    # ---- Visualization of the most recent simulation ----
    if sims:
        latest_id = sims[-1]['id']
        ...
        with st.expander("⚙️ 3D Visualization Controls", expanded=True):
            ...
            with tabs[0]:
                if has_snapshots:
                    ...
                    with col1:
                        time_idx = st.slider("Time Step", 0, n_snapshots-1,
                                             n_snapshots-1, key='time_slider_ms')
                    with col2:
                    ...
                    fixed_cscale = st.checkbox("Fixed color scale across time (recommended)", value=True, key='fix_cscale')
                    if fixed_cscale:
                    ...
                    fig_ms = create_multi_slice_3d_visualization(
                        ...
                        current_time=current_time, vmin=vmin, vmax=vmax)
                    ...
                    fig_ms = create_multi_slice_3d_visualization(
                        T_final, ext, advanced_styling, n_slices=5,
            ...
            with tabs[2]:
                ...
                Nx_, Ny_, Nz_ = T_final.shape
                X = np.linspace(ext['x'][0], ext['x'][1], Nx_)
                Y = np.linspace(ext['y'][0], ext['y'][1], Ny_)
                Z = np.linspace(ext['z'][0], ext['z'][1], Nz_)
                Xg, Yg, Zg = np.meshgrid(X, Y, Z, indexing='ij')
                # v3.0: dynamic isosurface levels spanning the actual T range
                T_lo, T_hi = float(T_final.min()), float(T_final.max())
                span = max(T_hi - T_lo, 1.0)
                iso_levels = np.linspace(T_lo + 0.15*span, T_hi - 0.05*span, 6)
                fig_iso = go.Figure()
                for lvl in iso_levels:
                    fig_iso.add_trace(go.Isosurface(
                        x=Xg.flatten(), y=Yg.flatten(), z=Zg.flatten(),
                        value=T_final.flatten(),
                        isomin=lvl-0.01*span, isomax=lvl+0.01*span,
                ...
                fig_iso.update_layout(
                    scene=dict(xaxis=dict(title='x (m)'), yaxis=dict(title='y (m)'),
                               zaxis=dict(title='z (m)'), aspectmode='data'),
                    title=dict(text=f'🔥 Isosurfaces (Smooth) | T {T_lo:.0f}–{T_hi:.0f} K', x=0.5),
                    height=700)
        ...
        # ---- 2D Time Evolution Slider (upgraded: global color scale + cmap) ----
        if len(sim_data['history']) > 1:
            st.subheader("⏳ 2D Time Evolution Slider")
            frames = []
            gmin = min(h['T_mid'].min() for h in sim_data['history'])
            gmax = max(h['T_mid'].max() for h in sim_data['history'])
            slider_cscale = matplotlib_to_plotly(advanced_styling.get('cmap', 'hot'), 20)
            for entry in sim_data['history']:
                frames.append(go.Frame(
                    data=[go.Heatmap(z=entry['T_mid'], colorscale=slider_cscale,
                                     zmin=gmin, zmax=gmax)],
                    name=f"t={entry['time']:.1f}s"))
            fig_slider = go.Figure(
                data=[go.Heatmap(z=sim_data['history'][0]['T_mid'],
                                 colorscale=slider_cscale, zmin=gmin, zmax=gmax,
                                 colorbar=dict(title='T (K)', tickformat='.0f'))],
                frames=frames)
            ...
            if st.sidebar.button("🔬 Run Comparison", type="primary"):
                ...
                with st.expander("📊 Simulation Metadata"):
                    df_meta = pd.DataFrame([{
                        ...
                        'Abuse': s['metadata'].get('abuse_mode', '?'),
        ...
        if export_format == "VTK for ParaView (.vts/.pvd)":
            ...
            with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as zf:
                ...
                for sim_id, sim_data in all_sims.items():
                    ...
                    pvd_lines.append(f'    <DataSet timestep="{t}" group="" part="" file="{vts_filename}"/>')
...
# -----------------------------------------------------------------------------
# 14. Theoretical Documentation (updated for v3.0)
# -----------------------------------------------------------------------------
with st.expander("🔬 Theoretical Soundness & Advanced Analysis", expanded=False):
    st.markdown("""
    **Multi‑Stage Arrhenius Kinetics (v3.0 – conversion convention)**
    - **SEI decomposition**: 1st order, f(c) = (1‑c), exact exponential update
    - **Anode/electrolyte**: autocatalytic c(1‑c) with physical nucleation seed c0 = 1e‑4
    - **Separator & cathode**: 1st order; cathode activates > 500 K
    - Heat release is **energy conserving**: q = Σ Hᵣ·Δcᵣ/Δt (ΣH ≈ 1.42e9 J/m³ → ΔT_ad ≈ 550 K)
    - Calibrated against Zhang et al. 2024: T_onset 70–135 °C, dT/dt > 1 K/s at TR,
      Tmax 600–900 K, ~65% of self‑heat released after TR.

    **Abuse Protocols (simulation conditions)**
    - **Lateral heater**: sustained flux q0 on x=0 face (paper Eq. 2), auto‑off at cutoff →
      reproduces Stages I–IV and cell‑to‑cell propagation.
    - **ARC adiabatic**: bulk pre‑heat, h = ε = 0 → onset statistics comparable to Table 2.
    - **Internal hotspot**: seeded spherical trigger for ISC/nail studies (needs ≥ kJ trigger energy).
    ...
    **Boundary Conditions**
    ...
    - Variable h_conv simulates natural convection (h≈7‑15) or near‑adiabatic (h≈0‑5)

    **Multi‑Simulation Value**
    - Compare cooling strategies, hotspot severity, heater power
    - Quantify TR onset time and triggering energy vs. paper's 20.8% criterion
    """)

st.caption("🔥 Multi‑Simulation Thermal Runaway Platform • v3.0 • calibrated kinetics + real abuse protocols + fixed visualization")
