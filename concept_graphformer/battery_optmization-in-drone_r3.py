...
# -*- coding: utf-8 -*-
"""
Battery Optimization in Drone Systems Concept Graph v6.1 + QDWA
===============================================================================
...
# ============================================================================
st.set_page_config(
    page_title="Battery Optimization in Drone Systems Concept Graph v6.1 + QDWA",
    page_icon="🔋",
    layout="wide",
...
def build_query_whitelist(st_session):
    ...
    if (
        ...
        whitelist.update(
            ...
            if categorize_battery_concept(c) == k_star
        )
...
# ============================================================================
FOUR_CATEGORIES = ["hardware", "flight_control", "thermal", "maintenance"]
CATEGORY_DISPLAY = {
    "hardware": "Hardware & Architecture",
    "flight_control": "Flight Control & Trajectory",
    "thermal": "Thermal Management",
    "maintenance": "Charging & Maintenance",
}
_LABEL2KEY = {
    "Hardware & Architecture": "hardware",
    "Flight Control & Trajectory": "flight_control",
    "Thermal Management": "thermal",
    "Charging & Maintenance": "maintenance",
}
...
PROBLEM_TO_CATEGORY = {
    BatteryProblem.BATTERY_CHEMISTRY_SELECTION: "hardware",
    BatteryProblem.BMS_OPTIMIZATION: "hardware",
    BatteryProblem.PATH_PLANNING: "flight_control",
    BatteryProblem.MOTOR_EFFICIENCY: "flight_control",
    BatteryProblem.THERMAL_MANAGEMENT: "thermal",
    BatteryProblem.CYCLE_LIFE_EXTENSION: "maintenance",
    BatteryProblem.GENERAL: None,
    BatteryProblem.MULTI_PROBLEM: None,
}

# Blocks b(i) for the 32 LatentMoE experts (order = BATTERY_EXPERT_LABELS)
EXPERT_CATEGORY = (
    ["hardware"] * 12
    + ["thermal"] * 4
    + ["flight_control"] * 9
...
@st.cache_resource(show_spinner=False)
def get_category_anchors() -> Tuple[List[str], np.ndarray]:          # Eq. (1)
...
def soft_category_membership(term: str, beta: float = 8.0) -> Dict[str, float]:   # Eq. (2)
    try:
        ...
        hard = categorize_battery_concept(term)
...
def concentration_of(W: Dict[str, float]) -> float:                 # Eq. (5)
...
def compute_query_category_weights(
    query: str,
    primary_problem: Optional[BatteryProblem] = None,
) -> Tuple[Dict[str, float], float, int]:                           # Eqs. (3),(4),(5),(7)
    q = query.lower()
    raw = {k: 0.0 for k in FOUR_CATEGORIES}

    # 1. Keyword-based problem detection (GENERAL excluded -> fixes dilution)
    for p, kws in FallbackAnalyzer.PROBLEM_KEYWORDS.items():
        if p == BatteryProblem.GENERAL:
            continue
    ...
    # 3. Soft term evidence from extracted concepts
    for term in extract_battery_concepts_from_text(q):
        for k, m in soft_category_membership(term).items():
            raw[k] += m

    W = allocate_category_weights(raw)
    c = concentration_of(W)
    K = 2 + int(round(2 * c))                                      # K in {2,3,4}
    return W, c, K
            ...
            for n in frontier:
                for nb in self.full_graph.neighbors(n):
                    if nb in selected_nodes:
                        continue
                    ncat = {categorize_battery_concept(x) for x in self.full_graph.neighbors(nb)}
                    if categorize_battery_concept(nb) == k_star or (len(ncat) >= 2 and self.full_graph.degree(nb) > 2):
                        nxt.add(nb)
            selected_nodes |= nxt
            frontier = nxt
...
QDWA_CATEGORIES = {
    "Hardware & System Architecture": [
        ...
        "energy_density", "c_rate", "internal_resistance", "battery_management_system",
        "cell_balancing", "state_of_charge", "state_of_health", "voltage_sag",
        "airframe_weight", "payload_management", "carbon_fiber_airframe",
    ],
    "Flight Control & Trajectory Optimization": [
        "throttle_management", "energy_aware_path_planning", "a_star_algorithm",
        "genetic_algorithm", "cruising_velocity", "motor_efficiency",
        "propeller_matching", "aerodynamic_drag", "altitude_management",
    ],
    "Thermal Management": [
        "thermal_management", "pre_heating", "post_flight_cooling",
        "thermal_runaway", "capacity_degradation", "temperature_effect_on_ir",
    ],
    "Charging & Maintenance Protocols": [
        "storage_soc", "charge_rate", "discharge_threshold",
        "cycle_life", "rotation_management",
    ],
}

CATEGORY_SHORT_NAMES = {
    "Hardware & System Architecture": "Hardware",
    "Flight Control & Trajectory Optimization": "Flight Algo",
    "Thermal Management": "Thermal",
    "Charging & Maintenance Protocols": "Charging",
}
...
class QDWAWeightAllocator:
    ...
    Given a user query and a pre-built DomainOntology with embeddings,
    computes a 4-category weight vector (α₁…α₄) that sums to 1.
    """
    ...
    def compute_weights(self, query: str) -> Dict[str, float]:
        ...
        Returns dict  {category_full_name: weight_float}.
        """
    ...
    def get_major_category(self) -> Tuple[str, float]:
        """Return (category_name, weight) of the highest-weight category."""
    ...
    def weights_dataframe(self) -> pd.DataFrame:
        """Return a tidy DataFrame for display / plotting."""
        rows = []
...
# ============================================================================
def main() -> None:
    st.title("🔋 Battery Optimization in Drone Systems Concept Graph v6.1 + QDWA")
    st.caption(
        "Multi-level reasoning concept graph for battery optimization in drone systems | "
